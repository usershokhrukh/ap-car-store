...
const AdminsTable = ({data, adminData}) => {
  ...
  const {setCoords, setText, setOpenTooltip} = useContext(TooltipContext);
      ...
      <tbody className="products__t-body">
        {data?.data?.items?.map(
          ({login, fullName, isSuperAdmin, updatedAt, id}) => (
            <tr
              style={{
                backgroundColor: `${id == adminData?.data?.id ? "rgb(from var(--app-cyan) r g b / 0.05)" : ""}`,
                border: `${id == adminData?.data?.id ? "1px solid rgb(from var(--app-cyan) r g b / 0.2)" : ""}`,
              }}
              key={`${name} ${id}`}
            ...
            >
              ...
              {adminData?.data?.isSuperAdmin ? (
                <td className="products__t-b-td disable-admin-td">
                  {!isSuperAdmin ? (
                    <>
                      <button
                        onClick={() => {
                ...
                <td className="products__t-b-td disable-admin-td">
                  <DisableAdmin
                    ...
                    text={"System Admin can not change admins"}
                  />
                </td>
              )}
            </tr>
