"use client";

import { useEffect } from "react";
import { MapContainer, TileLayer, Marker, Popup, useMap } from "react-leaflet";
import L from "leaflet";
import "./map.modules.scss"

const svgIcon = L.divIcon({
  html: `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M18.364 17.364L12 23.7279L5.63604 17.364C2.12132 13.8492 2.12132 8.15076 5.63604 4.63604C9.15076 1.12132 14.8492 1.12132 18.364 4.63604C21.8787 8.15076 21.8787 13.8492 18.364 17.364ZM12 15C14.2091 15 16 13.2091 16 11C16 8.79086 14.2091 7 12 7C9.79086 7 8 8.79086 8 11C8 13.2091 9.79086 15 12 15ZM12 13C10.8954 13 10 12.1046 10 11C10 9.89543 10.8954 9 12 9C13.1046 9 14 9.89543 14 11C14 12.1046 13.1046 13 12 13Z"></path></svg>
  `,
  className: "map-svg",
  popupAnchor: [0, -40],
});

// Dynamic component that forces the Leaflet map view to pan when props change
function ChangeMapView({ center }) {
  const map = useMap();
  useEffect(() => {
    if (center && center[0] && center[1]) {
      map.invalidateSize(); // Fixes gray loading squares/glitches
      map.flyTo(center, map.getZoom(), { animate: true, duration: 1.2 });
    }
  }, [center, map]);
  return null;
}

export default function ProductMap({data}) {
  const position = [data.latitude, data.longitude];

  return (
    <div 
    className="map__wrapper"
    >
      <MapContainer
        center={position}
        zoom={15}
        scrollWheelZoom={true}
        style={{width: "100%", height: "100%"}}
      >
        <TileLayer
          attribution='&copy; <a href="https://openstreetmap.org">OpenStreetMap</a> contributors'
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
        />

        {/* This updates the map position when data changes dynamically */}
        <ChangeMapView center={position} />

        <Marker position={position} icon={svgIcon}>
          <Popup>
            <div className="map__pop-box">
              <strong>{data.name}</strong>
              <br />
              {data.address}, {data.city}
              <br />
              Work schedule: {data.opensAt} - {data.closesAt}
              <br />
              Tel: {data.phone}
            </div>
          </Popup>
        </Marker>
      </MapContainer>
    </div>
  );
}
