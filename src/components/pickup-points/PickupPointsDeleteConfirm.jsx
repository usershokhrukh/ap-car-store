...
import { useDeletePickup } from "@/hooks/pickup/DELETE/DeletePickup";
import {useNotify} from "@/hooks/useNotify";
import {useRouter} from "next/navigation";
import React, {useContext, useEffect} from "react";

const PickupDeleteConfirm = ({id}) => {
  const route = useRouter();

  const {setCloseModal, setModalStopped} = useContext(GeneralModal);

  const {error, mutate, data, isPending} = useDeletePickup();
  ...
  return (
    <form onSubmit={handleSubmit} className="modal__form">
      {!isPending && !data?.success ? (
        <>
          <p className="modal__title">Are you sure to delete pickup point?</p>
          <button
            type="submit"
...
export default PickupDeleteConfirm;
