...
const ProductsOneView = () => {
  ...
  const {
    data: patchCategoryData,
    error: patchCategoryError,
    isPending: patchCategoryPending,
    mutate: patchCategoryMutate,
  } = usePatchStatusCategories();
  useEffect(() => {
    if (patchCategoryError?.message) {
      notice({
        text: patchCategoryError?.message,
        status: "error",
        time: "infinite",
        close: true,
      });
  ...
  useEffect(() => {
    if (
      patchCategoryData &&
      !patchCategoryPending &&
      !patchCategoryError?.message
    ) {
      notice({
        text: patchCategoryData?.message,
        time: 5000,
        status: "success",
      });
    }
  }, [patchCategoryData, patchCategoryPending]);
