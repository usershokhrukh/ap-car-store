...
const request = async (payload) => {
  try {
    const res = await api.patch(`api/products/${payload[0]}/status`, payload[1]);
    return res?.data;
  } catch (error) {
...
export const usePatchStatusProducts = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({queryKey: ["pickup-products"]})
    },
  });
};
